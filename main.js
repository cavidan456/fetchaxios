...
function idControl(param) {
    orders.filter((item)=>{
        if(item.id===param){
            console.log(item.shipAddress.country);
        }
    })
}

//2.Ölkəyə görə obyekti tapa function yazın.
function countrymall(element) {
    orders.filter(item =>{
        for (let i = 0; i < item.length; i++) {
            if (item[i].id === element) {
                console.log(item[i]);
            }
        }
    })
...
function idSearch(a) {
    orders.filter((item)=>{
        if(item.id===a){
            console.log("shipName:"+ item.shipName+ " " + "country:" + item.shipAddress.country+ " " +"City:"+ item.shipAddress.city);
        }
    })
}
...
function detailsSearch(ID) {
    orders.filter((item)=>{
        let cem = 0
        if (item.id===ID) {
            for (let i = 0; i < item.details.length; i++) {
                let total = item.details[i].unitPrice + item.details[i].discount
                cem = cem + total;
            }
             console.log(cem); 
        } 
